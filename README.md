...
| [ashare-data-kit](skills/ashare-data-kit/) | A-share (China stock market) full-stack data toolkit — seven data layers (quotes, research reports, sentiment signals, capital flow, news, fundamentals, announcements) exposed as `uv`-run, PEP 723 self-installing scripts under `scripts/`, with progressive-disclosure reference docs. Covers valuation, report search, theme attribution, dragon-tiger-list tracking, lockup-release alerts, sector rotation, margin trading, chip concentration, supply-chain research, and batch screening. |
